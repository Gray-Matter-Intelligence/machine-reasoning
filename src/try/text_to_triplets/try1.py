# Relates to ISSUE: https://github.com/Gray-Matter-Intelligence/machine-reasoning/issues/2
...
WIKIDATA_PREFIX_E = 'wd:'
WIKIDATA_PREFIX_P = 'wdt:'

# Important wikidata properties
WIKIDATA_P_SUBCLASS_OF = 'P279'
WIKIDATA_P_INSTANCE_OF = 'P31'
...
def get_typeof(item: str, subclass=False):
    ...
    rel_type = WIKIDATA_P_SUBCLASS_OF if subclass else WIKIDATA_P_INSTANCE_OF
    try:
        r = wikidata_sparql_api(f'''SELECT ?class
    WHERE
    {{
    {WIKIDATA_PREFIX_E}{item} {WIKIDATA_PREFIX_P}{rel_type} ?class
    }}''')
